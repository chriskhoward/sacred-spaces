...
const DEFAULT_IMAGINE_CARDS = [
  { image: null, text: 'Teach with confidence, knowing your faith and practice are not in conflict.', fallbackSrc: '/assets/images/1.webp' },
  { image: null, text: 'Be surrounded by teachers who understand the nuance of your journey.', fallbackSrc: '/assets/images/2.webp' },
  { image: null, text: 'Have access to opportunities that support both your spiritual and professional growth.', fallbackSrc: '/assets/images/3.webp' },
];

const DEFAULT_BENEFIT_CARDS = [
  { image: null, heading: 'Teacher Directory Placement (Value: $297/yr.)', description: 'Visibility through our publicly accessible directory so students, churches, and organizations can find and hire Christ-Centered Yoga Teachers of Color worldwide.', fallbackSrc: '/assets/P3WuMgPVZVr3eZ2SvkCVrEnYHe71HnA6TlnFH1R3.webp' },
  { image: null, heading: 'Monthly Community Check-Ins (Value: $480/yr.)', description: 'Virtual gatherings for connection, reflection, networking, and shared growth — facilitated with intention and care.', fallbackSrc: '/assets/Monthly Community Check-Ins (Value- $).webp' },
  { image: null, heading: 'Private Community Space (Value: $360/yr.)', description: 'A private online community space where we connect daily, ask questions, share wins, swap resources, and collaborate — without having to explain or defend our identity.', fallbackSrc: '/assets/Private Community Space (Value- $).webp' },
  { image: null, heading: 'Quarterly Workshops & Masterclasses (Value: $600/yr.)', description: 'Spiritually aligned and culturally grounded workshops led by experienced teachers and industry leaders on business growth, theology, yoga integration, trauma-informed teaching, and more.', fallbackSrc: '/assets/Quarterly Workshops & Masterclasses (Value- $).webp' },
];

const DEFAULT_PREMIUM_ITEMS = [
  { heading: 'PROMOTION OF YOUR\nOFFERINGS', description: 'Your classes, workshops, retreats, or courses shared through Flow in Faith social channels and newsletters.', value: '(Value: $600/yr.)' },
  { heading: 'PAID TEACHING\nOPPORTUNITIES', description: 'Opportunities to teach Christ-Centered yoga classes or workshops inside the Flow in Faith Sanctuary Membership.', value: '(Value: $1,200/yr.)' },
  { heading: 'CONTRIBUTION TO THE\nON-DEMAND LIBRARY', description: 'Share your Christ-Centered recorded classes, meditations, or resources to reach students globally.', value: '(Value: $500/yr.)' },
];

const DEFAULT_VISIONARIES = [
  {
    name: 'Queen Robertson',
    role: 'Founder/Visionary',
    photo: null,
    fallbackSrc: '/images/homepage/founder-queen-new.png',
    bioParagraphs: [
  ...
  },
  {
    ...
    role: 'Founder/Visionary',
    photo: null,
    fallbackSrc: '/images/homepage/founder-de-new.png',
    bioParagraphs: [
...
export async function generateMetadata(): Promise<Metadata> {
  let tc: TeacherCollectivePageData | null = null;
  try {
    tc = await client.fetch<TeacherCollectivePageData | null>(TC_PAGE_QUERY);
  } catch {
    // use defaults
  }
  return {
    title: tc?.metaTitle || 'Teachers Collective | Flow in Faith',
    description: tc?.metaDescription || 'A community-centered home for Yoga Teachers of Color who identify as Christian. Find support, professional development, and spiritually grounded connection.',
  };
}
...
export default async function TeacherCollectivePage() {
  ...
  const [tcData, faqsData] = await Promise.all([
    client.fetch<TeacherCollectivePageData | null>(TC_PAGE_QUERY),
    client.fetch<typeof DEFAULT_FAQS>(TC_FAQS_QUERY),
  ]);
  const tc = tcData || ({} as TeacherCollectivePageData);
  if (faqsData?.items?.length) faqs = faqsData;

  const filloutId = tc.filloutId || 'e4pZwnuAkYus';

  const imagineCards = tc.imagineCards?.length
    ? tc.imagineCards.map((card, i) => ({
        src: card.image ? urlForImage(card.image).width(800).url() : DEFAULT_IMAGINE_CARDS[i]?.fallbackSrc || '/assets/images/1.webp',
        text: card.text || DEFAULT_IMAGINE_CARDS[i]?.text || '',
      }))
    : DEFAULT_IMAGINE_CARDS.map((card) => ({
        src: card.fallbackSrc,
        text: card.text,
      }));

  const benefitCards = tc.benefitCards?.length
    ? tc.benefitCards.map((card, i) => ({
        src: card.image ? urlForImage(card.image).width(800).url() : DEFAULT_BENEFIT_CARDS[i]?.fallbackSrc || '',
        heading: card.heading || DEFAULT_BENEFIT_CARDS[i]?.heading || '',
        description: card.description || DEFAULT_BENEFIT_CARDS[i]?.description || '',
      }))
  ...
  const premiumItems = tc.premiumItems?.length
    ? tc.premiumItems.map((item, i) => ({
        heading: item.heading || DEFAULT_PREMIUM_ITEMS[i]?.heading || '',
        description: item.description || DEFAULT_PREMIUM_ITEMS[i]?.description || '',
        value: item.value || DEFAULT_PREMIUM_ITEMS[i]?.value || '',
      }))
    : DEFAULT_PREMIUM_ITEMS;

  const visionaries = tc.visionaries?.length
    ? tc.visionaries
    : null;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
            <div className="text-center lg:text-left">
              ...
              <div
                ...
                data-fillout-button-text={tc.heroCtaText || 'Join the Teachers Collective'}
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl lg:text-4xl font-bold text-white mb-12 text-center">
              {tc.imagineHeading || 'Imagine what it would feel like to…'}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {imagineCards.map((card, idx) => (
                <div key={idx} className="bg-white rounded-lg overflow-hidden shadow-lg border-2 border-(--color-roti)">
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white p-10 md:p-12 rounded-lg border border-gray-200">
              <h2 className="text-2xl lg:text-3xl font-bold text-(--color-roti) mb-8 text-center">
                {tc.listenHeading || 'Listen, I see you…'}
              </h2>
              <div className="space-y-6 text-gray-700 leading-relaxed">
                {tc.listenBody?.length ? (
                  <PortableText value={tc.listenBody} />
                ) : (
                  <>
                    <p>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="relative aspect-[4/5] w-full max-w-md mx-auto lg:mx-0">
                <Image
                  src={tc.youDontHaveToImage ? urlForImage(tc.youDontHaveToImage).width(800).url() : '/images/homepage/community-image.jpg'}
              ...
              <div>
                <h2 className="text-3xl lg:text-4xl font-bold text-(--color-primary) mb-8">
                  {tc.youDontHaveToHeading || "You don\u2019t have to…"}
                </h2>
                <div className="space-y-6">
                  {(tc.youDontHaveToItems?.length
                    ? tc.youDontHaveToItems
                    : [
      ...
      <section className="py-16 bg-(--color-primary)">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-2xl lg:text-3xl font-bold text-white mb-6 max-w-2xl mx-auto leading-relaxed">
            {tc.callingHeading || 'Your calling deserves to be held, affirmed, and supported in community.'}
          </h2>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="mb-12">
              <div className="relative w-full max-w-4xl mx-auto">
                <Image
                  src={tc.memberBannerImage ? urlForImage(tc.memberBannerImage).width(1200).url() : '/assets/images/tc_hero_banner.png'}
            ...
            <h2 className="text-[38px] font-black text-(--color-primary) mb-2 text-center leading-[1.25]">
              {tc.memberHeading || 'As a member of Flow in Faith Teachers Collective'}
            </h2>
            <p className="text-lg text-gray-600 mb-12 text-center">{tc.memberSubtext || "you\u2019ll have access to:"}</p>

            {/* Benefits Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
              {benefitCards.map((card, idx) => (
                <div key={idx} className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden p-6">
                  <div className="relative w-full aspect-[16/9] mb-6">
                    <Image src={card.src} alt={card.heading} fill className="object-contain" />
                  </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl lg:text-[38px] font-black text-(--color-roti) mb-10 text-center tracking-wider uppercase">
              {tc.premiumHeading || 'PLUS THESE PREMIUM OPPORTUNITIES:'}
            </h2>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl lg:text-4xl font-bold text-(--color-primary) mb-4 text-center">
              {tc.membershipHeading || 'Membership Options'}
            </h2>
            <p className="text-xl text-gray-600 mb-12 text-center">
              {tc.membershipSubtext || 'Choose the path that best supports your journey.'}
            </p>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-[38px] font-bold leading-[1.25] text-center text-white mb-12">
              {tc.itsTimeToHeading || "It\u2019s Time to.."}
            </h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-12">
              <div className="relative aspect-video w-full max-w-md mx-auto lg:mx-0 rounded-lg overflow-hidden">
                <Image
                  src={tc.itsTimeToImage ? urlForImage(tc.itsTimeToImage).width(800).url() : '/assets/images/time%20to.jpg'}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {visionaries ? (
                visionaries.map((v, idx) => (
                  <div key={idx} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 md:p-10">
                    <div className="flex justify-center mb-6">
                      <div className="relative w-56 h-56 md:w-64 md:h-64 rounded-full border-4 border-(--color-roti) overflow-hidden">
                        <Image
                          src={v.photo ? urlForImage(v.photo).width(500).height(500).url() : DEFAULT_VISIONARIES[idx]?.fallbackSrc || '/images/homepage/founder-queen-new.png'}
                          alt={v.name || ''}
                          fill
                          className="object-cover"
                        />
                    ...
                    <h4 className="text-2xl font-bold text-(--color-primary) mb-1 text-center">{v.name}</h4>
                    <p className="text-(--color-roti) font-semibold mb-5 text-center">{v.role || 'Founder/Visionary'}</p>
                    <div className="text-gray-700 space-y-4 text-sm leading-relaxed">
                      {v.bio?.length ? (
                        <PortableText value={v.bio} />
                      ) : (
                        DEFAULT_VISIONARIES[idx]?.bioParagraphs.map((p, pi) => (
                          <p key={pi}>{p}</p>
                        ))
                      )}
                    </div>
      ...
      <section className="py-16 bg-(--color-primary)">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl lg:text-[42px] font-bold text-white mb-4 leading-tight">
            {tc.finalCtaHeading || 'So, what do you say?'}
          </h2>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
              ...
              <div className="relative aspect-[4/3] w-full rounded-lg overflow-hidden">
                <Image
                  src={tc.bottomLineImage ? urlForImage(tc.bottomLineImage).width(800).url() : '/images/homepage/bottom-line-yoga.png'}
              ...
              <div className="text-center lg:text-left">
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-(--color-roti) mb-4 italic leading-tight">
                  {tc.bottomLineHeading || 'Bottom line… There is space for you here!!'}
                </h2>
